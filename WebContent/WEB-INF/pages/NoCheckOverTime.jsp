...
<c:url value="/resources/js/Project/RealTime.Modify.NoCheckOvertime.js?version=${resourceVersion}" var="modifyNoCheckOvertimeJS" />
		...
		<div style="top: 55px; margin-left: 10px">
			<div class="panel-body" style="border: 1px solid #e1e3e6;">
				...
				<div class="col-sm-12 col-sm-12" style="margin: 5px;">
					...
					<input id="endDate" class="Wdate" type="text" onClick="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'startDate\')}',maxDate:'%y-%M-\#{%d-1}'})" autocomplete="off" />
				</div>
		
				<div class="col-sm-12 col-sm-12" style="margin: 5px;">
					<input type="button" id="searchNoCheckOvertimeBtn" name="searchNoCheckOvertimeBtn" class="btn btn-sm btn-primary" value="查詢">
					<input type="button" id="showAllNoCheckOvertimeBtn" name="showAllNoCheckOvertimeBtn" class="btn btn-sm btn-primary" value="顯示全部">
					<a href="#" class="easyui-linkbutton" id="exportExcel" data-options="iconCls:'icon-search'"></a>
					<input type="button" onclick="javascript:Save1('未提報加班人員記錄')" name="exportNoCheckOvertimeBtn" class="btn btn-sm btn-primary" value="導出">
					<input type="button" id="resetBtn" name="resetBtn" class="btn btn-sm btn-primary" value="清空">
				<div>
					<div>
						<h4>未提報加班人員查詢列表：</h4>  <label id="searchNoCheckOvertimeCounts">記錄數: 0  條</label>
					</div>
					<div class="panel-body" style="border: 1px solid #e1e3e6; width: 100%">
						<table id="NoCheckOvertimeTable" class="table table-striped">
							<thead>
								<tr>
									<th>工號</th>
									<th>姓名</th>
				...
				<div id="NoCheckOvertimePagination" align="right" style="height: 20">
				</div>
			</div>
		</div>
	</div>
