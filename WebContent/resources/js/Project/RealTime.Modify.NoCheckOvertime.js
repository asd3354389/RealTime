$(document).ready(function(){
	...
	$('#uploadCostIdFile').change(function(){  
     ...
     $("#exportNoCheckOvertimeBtn").click(function() {
		
		$("#NoCheckOvertimeTable").table2excel(
				{
	...
	$('#searchNoCheckOvertimeBtn').click(function(){	
		SearchNoCheckOvertimes(false);
	});
	
	$('#showAllNoCheckOvertimeBtn').click(function(){	
		SearchNoCheckOvertimes(true);
	});
	
	function SearchNoCheckOvertimes(isShowAll){	
		...
		if(errorMessage==''){
			getAsyncNoCheckOvertimeInfo(curPage,empId,depId,costId,startDate,endDate,isShowAll);
		}else{
	...
	function ShowSearchNoCheckOvertimeTable(rawData,isShowAll){
		$('#NoCheckOvertimeTable tbody').empty();
		...
		if(totalRecord>0){
			$('#searchNoCheckOvertimeCounts').html("");
			$('#searchNoCheckOvertimeCounts').html("記錄數: "+totalRecord+" 條");;
		}
		...
		for(var i=0;i<executeResult.length;i++){
			...
			'<td style="mso-number-format:\'\@\';" ng-bind="data.paySerialNo">'+executeResult[i]["SwipeCardDate"]+'</td>'+
			'<td>'+goWorkcurTime+'</td>'+
			'<td>'+endWorkcurTime+'</td>'+
			'<td style="mso-number-format:\'\@\';" ng-bind="data.paySerialNo">'+subOnWorkTime+'</td>';
			
			$('#NoCheckOvertimeTable tbody').append(tableContents);			
		}	
		if(!isShowAll){
			refreshNoCheckOvertimeInfoPagination(currentPage,totalRecord,totalPage,pageSize);
		}
		else{
			$('#NoCheckOvertimePagination').empty();
		}
	}
	...
	function refreshNoCheckOvertimeInfoPagination(currentPage,totalRecord,totalPage,pageSize){
		$('#NoCheckOvertimePagination').empty();
	  ...
	  /*  for(var i=1;i <= totalPage;i++){
		...
		$('#NoCheckOvertimePagination').append(paginationElement);
		
		$('.firstPage').click(function(){
			curPage=1;
			getAsyncNoCheckOvertimeInfo(curPage,empId,depId,costId,startDate,endDate,false);			
		});
	...
	function getAsyncNoCheckOvertimeInfo(curPage,empId,depId,costId,startDate,endDate,isShowAll){
		$.ajax({
			type:'POST',
			url:'../NoCheckOvertime/SearchNoCheckOvertime.show',
			async:false,
			...
			success:function(rawData){
				...
				if (rawData != null && rawData != "") {
					...
					if(errorResponse!=null){
						...
						if(numOfRecords>0)
							ShowSearchNoCheckOvertimeTable(rawData,isShowAll);
						else
							alert('找不到資料');
					}
				}
...
function Save1(name){
	...
	+ new Date().getHours() + ":"
	+ new Date().getMinutes();
	$('#NoCheckOvertimeTable').tableExport({type:'excel'},filename);
};