$(document).ready(function(){
		...
		if($('#deleteId .dlTable').children().length==0){
			...
			$('#deleteId .dlTable').find('tr').each(function(i,e){
				//				console.log(i);
								...
								dltr.deviceIP = child.eq(1).text();
								dltr.emp_id = child.eq(0).text();
								relist.push(dltr);
			})
			console.log(relist);
			...
			if(results==true){
				$.ajax({
					...
					url:'../DeparturePerson/deleteDeparturePerson.do',
	...
	$('#setEmpIPBinding').click(function(){
		...
		if(errorMessage==''){
			...
			$.ajax({
				...
				url:'../DeparturePerson/AdddeparturePerson.do',
				data:JSON.stringify(EmpIpBinding),
				dataType:'json',
				success:function(data){
					//alert("返回代码"+data.StatusCode);
	...
	function ShowAllEmpIPBinding(){
		$.ajax({
			type:'POST',
			url:'../DeparturePerson/ShowAllDeparturePerson',
...
//				alert('找不到資料');
	...
	function ShowAllEmpIPBindingTable(rawData){
		...
		for(var i=0;i<executeResult.length;i++){
			var	tableContents='<tr><td class="touch">'+executeResult[i]["emp_id"]+'</td>'+
					'<td>'+executeResult[i]["name"]+'</td>'+
//					'<td>'+executeResult[i]["Direction"]+'</td>'
//					'<td>'++'</td>'+
					'<td><input type="button" value="" class="editBtn btn btn-xs btn-link"></td>';
				tableContents+='</tr>';
...
//			$(parentElement).children().find('.editBtn .deleteBtn').hide();
	...
	function getPersonList(curPage,queryCritirea,queryParam){
		$.ajax({
			type:'POST',
			url:'../DeparturePerson/ShowDeparturePersonList',
			data:{curPage:curPage,queryCritirea:queryCritirea,queryParam:queryParam},
			error:function(e){
				alert('找不到資料');
			},
