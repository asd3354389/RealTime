...
@Repository
public class FourteenROPTXDAO extends DAO<FourteenROP>{
	...
	public int getTotalRecord(String queryCritirea, String queryParam, String updateUser, String userDataCostId) {
		int totalRecord=-1;
    	String sSQL = "select count(*) from SWIPE.CSR_14R1_PERSONAL_TX t where t.enabled = 'Y' ";
    	try {
	...
	public List<FourteenROP> FindAllRecords(int currentPage, int totalRecord, String queryCritirea, String queryParam,
		...
		String sSQL = "select * from (select b.*,rownum rn from "
				+ "(select t.id,t.start_date,t.end_date from SWIPE.CSR_14R1_PERSONAL_TX t "
				+ " where t.enabled = 'Y' ";
		try {
			List <Object> queryList=new  ArrayList<Object>();
			if(queryCritirea.equals("id")){
	...
	public boolean DeleteFourteenROP(String id, String updateUser, String startDate, String endDate) {
		...
		String sSQL="update CSR_14R1_PERSONAL_TX t set t.enabled = 'N',t.update_userid = ?,t.update_time = sysdate "
	...
	public boolean addfourteenROP(FourteenROP[] fourteenROP, String updateUser) {
		...
		String ISQL="insert into CSR_14R1_PERSONAL_TX values(UPPER(trim(?)),?,?,?,sysdate,'Y')";
		try {
			if(fourteenROP!=null) {
				/*
	...
	public List<Emp> GetPersonById(String costId) {
		...
		String sSQL = "select * from (select id,name,depid,costid from SWIPE.CSR_EMPLOYEE where isonwork=0 and depid='"+costId+"')";
		try {
